...
#USPostalCodes
#WarehouseLocations
#transittimes --- Currently empty cluster
...
saveData(data = SampleData,collectionName = "SampleData")
...
loadData <- function(collectionName) {
  ...
  #data <- db$find(query = paste0('{"ColumnName": {"$in" : ["',FindWhatinColumnName,'"]}}'), fields = '{}')
  data <- db$find()
  data
}

deleteData <- function(collectionName) {
  # Connect to the database
  db <-
    mongo(
      collection = collectionName,
...
updateData <- function(collectionName, mongoID, df) {
  # Connect to the database
  db <-
    mongo(
      collection = collectionName,
  ...
  data <- db$update(query = mongoID, update = df)
}

removeData <- function(collectionName) {
  # Connect to the database
  db <-
    mongo(
      collection = collectionName,
...
#Load invoice data
raw.og = loadData(collectionName = "SampleData")


## Rename Columns 
names(raw.og)[names(raw.og) == 'Recipient_State'] <- "Recipient.State"
names(raw.og)[names(raw.og) == 'Recipient_Zip_Code'] <- "Recipient.Zip.Code"
names(raw.og)[names(raw.og) == 'Express_or_Ground_Tracking_ID'] <- "Express.or.Ground.Tracking.ID"

### Remove Duplicates
raw.og <- raw.og[!duplicated(raw.og$Express.or.Ground.Tracking.ID),]

raw.og <- head(raw.og, 2000)
...
warehouse.locations <- loadData(collectionName = "WarehouseLocations")
